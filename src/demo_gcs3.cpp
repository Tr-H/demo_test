...
// to be complete------------------
int send(demo_test::pos_data& xyz, float *data, int fd)
{
    //to be complete
    ROS_INFO("sending position_feedback to uav1: x=%.4f, y=%.4f, z=%.4f", xyz.pos[0], xyz.pos[2], -xyz.pos[1]);
	...
	mocap_data pos_data;
	pos_data.x = xyz.pos[0];
	pos_data.y = xyz.pos[2];
	pos_data.z = -xyz.pos[1];

    memcpy(buffer+6, &pos_data, sizeof(pos_data));
	buffer[2]=seq++;		
	buffer[18]=88;
	...
	if(write_n!=20)
	{
		printf("send position feedback error\n");
		return -1;
	}
...
//
    ...
    memcpy(buffer2+6, &xyz_sp, sizeof(xyz_sp));
	buffer2[2]=seq2++;		
	buffer2[22]=193;
	...
	if(write_n2!=24)
	{
		printf("send position setpoint  error\n");
		return -1;
	}
