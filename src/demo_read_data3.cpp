...
int main(int argc, char **argv)
	...
	while(ros::ok() && i_number >1)
		...
		delta_t = t1 - t2;
			...
			while (start_run && i_number_read_main >= 0 && ( delta_t >= (float)(*(_data_map+(i_number_read_main*5)+4)) || map_delta_r <= 0.05 || (map_old_r!=0 && new_old_r >= map_old_r && (map_delta_r < new_old_r && map_delta_r < 0.09)) ))
			{
					if(delta_t != 0)
					{
							 VEL_SP = VEL_SP *delta_t/(float)(*(_data_map+(i_number_read_main*5)+4)) ;
							 if (VEL_SP > 0.28)
							 	VEL_SP = 0.28;
							 if (VEL_SP < 0.22)
							 	VEL_SP = 0.22;
					...
					ROS_INFO("*****************************");
					ROS_INFO("phase : ... %d ...%.4f........",i_number_read_main+1,VEL_SP);
					ROS_INFO("*****************************");
