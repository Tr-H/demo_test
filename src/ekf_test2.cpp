    ...
    pose.header.stamp = xyz.header.stamp;//ros::Time::now();
	 // pose.header.frame_id = "fcu";
	  pose.pose.position.x = (double)xyz.pos[0];
  	  pose.pose.position.y = (double)(xyz.pos[1]);
	  pose.pose.position.z = (double)(xyz.pos[2]);
	  pose.pose.orientation.x = (double)(xyz.q[0]);
	  pose.pose.orientation.y = (double)(xyz.q[1]);
	  pose.pose.orientation.z = (double)(xyz.q[2]);
	  pose.pose.orientation.w = (double)(xyz.q[3]);
    lock.unlock();
}
...
{	
	...
	while(ros::ok())
	{
		...
		// {	
			...
			_ekf_leader.header.stamp=pose.header.stamp;
			 _ekf_leader.pos[0]= pose.pose.position.x;
		  	 _ekf_leader.pos[1]= pose.pose.position.y;
			 _ekf_leader.pos[2]= pose.pose.position.z;
				 lock.unlock();
