...
{
   ROS_INFO_ONCE("[ MOCAP ] DATE RECEIVED !!");
    boost::mutex::scoped_lock lock(pos_mutex);
    memcpy(&xyz, &pos, sizeof(pos));

      pose.header.stamp = ros::Time::now();
	 // pose.header.frame_id = "fcu";
	  pose.pose.position.x = (double)xyz.pos[0];
  	  pose.pose.position.y = (double)(xyz.pos[2]);
	  pose.pose.position.z = (double)(-xyz.pos[1]);
...
{   	
	ROS_INFO_THROTTLE(1,"rewrite command !!");
...
{	
	...
	while(ros::ok())
	{
		...
		// global_i ++; 
				ROS_INFO_ONCE("[ MOCAP ] DATE SENDING !!");
				mocap_send_pub.publish(pose);
			// }
			// else
			// {
			// 	ROS_INFO_THROTTLE(4,"mocap data timeout!! check ros_client");
			// }
		lock.unlock();
	loop_rate.sleep();
	}
...
{	
	...
	ros::Subscriber sub2 = nh.subscribe("/pos_write_topic1",10, &writecallback);
	...
	status_upd_pub = nh.advertise<demo_test::pos_status>("/pos_status_topic1",10);
	...
	while(ros::ok())
	{
		ROS_INFO_ONCE("[ mavros_send ] RUNING!!");
		loop_rate.sleep();
	}
	return 0;
}