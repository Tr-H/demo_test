    ...
    memcpy(&xyz, &pos, sizeof(pos));
	 ...
	 // pose.header.frame_id = "fcu";
	  pose.pose.position.x = (double)xyz.pos[0+3*(global_id-1)];
  	  pose.pose.position.y = (double)(xyz.pos[2+3*(global_id-1)]);
	  pose.pose.position.z = (double)(-xyz.pos[1+3*(global_id-1)]);
					...
					while(ros::ok())	
						{
							...
							boost::mutex::scoped_lock type_lock(type_mutex);
								boost::mutex::scoped_lock lock(pos_mutex);
									...
									// }
									if (global_type_temp.type == 4 && pose.pose.position.z <= -0.36f)
									{	
										ROS_INFO("|UAV%d| :: has been desire position, trun into 'NORMAL' mode",global_id);
										global_type_temp.type = 0;
									}
									if (global_type_temp.type == 2 && pose.pose.position.z >= -0.11f)
									{
...
{	char * node_name;
	...
	// raw_mocapsend_name = new char[sizeof("/uav_posefeedback/pose")];
	// memset(raw_mocapsend_name, 0, sizeof("/uav_posefeedback/pose"));
	// strcpy(raw_mocapsend_name,"/uav_posefeedback/pose");
	// char * mocap_topic_name = strcat(strings,argv[1]);
	// char * mocap_topic_name2 = strcat(mocap_topic_name,raw_mocapsend_name);
	// char * mocap_topic_name2 = new char[sizeof("uav_posefeedback/pose")];
	// memset(mocap_topic_name2, 0, sizeof("uav_posefeedback/pose"));
	// strcpy(mocap_topic_name2,"uav_posefeedback/pose");
	// // raw_codesend_name = new char[sizeof("/uav_poseset/code")];
	// // memset(raw_codesend_name, 0, sizeof("/uav_poseset/code"));
	// // strcpy(raw_codesend_name,"/uav_poseset/code");
	// // char * codesend_topic_name2 = strcat(mocap_topic_name,raw_codesend_name);
	// char * codesend_topic_name2 = new char[sizeof("uav_posefeedback/pose1")];
	// memset(codesend_topic_name2, 0, sizeof("uav_posefeedback/pose1"));
	// strcpy(codesend_topic_name2,"uav_posefeedback/pose1");

		// int i_argc;
	 //    for (i_argc=0;i_argc<argc;i_argc++)
		//     {
		//     	    ROS_INFO("argv%d:%s",i_argc,argv[i_argc]);
		//     }
	//char * node_name_send = strcat(argv[1],node_name);
	...
	mocap_send_pub = nh.advertise<geometry_msgs::PoseStamped>("uav_posefeedback/pose",10);
