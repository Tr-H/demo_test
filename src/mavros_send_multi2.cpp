   ...
   ROS_INFO_ONCE("|UAV%d| :: [ MOCAP ] DATE RECEIVED !!",global_id);
    ...
    memcpy(&xyz, &pos, sizeof(pos));
	 ...
	 // pose.header.frame_id = "fcu";
	  ...
	  pose.pose.orientation.x = (double)(xyz.q[1 + 4*(global_id-1)]);
	  pose.pose.orientation.y = (double)(xyz.q[2 + 4*(global_id-1)]);
	  pose.pose.orientation.z = (double)(xyz.q[3 + 4*(global_id-1)]);
	  pose.pose.orientation.w = (double)(xyz.q[0 + 4*(global_id-1)]);
    lock.unlock();
}
