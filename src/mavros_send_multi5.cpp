...
demo_test::pos_write_data2 code_time1;
...
void udpcallback(const demo_test::pos_data& pos)
{
   ROS_INFO_ONCE("|UAV%d| :: [ MOCAP ] DATE RECEIVED !!",global_id);
    ...
    memcpy(&xyz, &pos, sizeof(pos));
      ...
      pose.header.stamp = xyz.header.stamp;//ros::Time::now();
...
{   	
	...
	boost::mutex::scoped_lock possp_lock(pos_sp_mutex);
    	...
    	boost::mutex::scoped_lock type_lock(type_mutex);
    		if(global_type_temp.type == 1 && pos1.type == 4)
    		{
    			boost::mutex::scoped_lock possp_lock1(pos_sp_mutex);
    			boost::mutex::scoped_lock lock(pos_mutex);
    						 ref_data[0]=xyz.pos[0 + 3*(global_id-1)];
			 				 ref_data[1]=xyz.pos[2 + 3*(global_id-1)];
							 if(ref_data[2] > -0.45f)
							 {ref_data[2]=-0.5f;}
							 ref_data[3]=-1.57f;
				lock.unlock();
	...
	ROS_INFO_ONCE("|UAV%d| :: [ COMMAND ]  SENDING!!",global_id);
		...
		float tan_y = 0;
				ros::Rate loop_rate(10);
					while(ros::ok())	
			{					
				if (((ros::Time::now() - code_time1.header.stamp) <= ros::Duration(0.03)) && global_type_temp.type == 3)
					{
						uav_code.header.stamp = code_time1.header.stamp;
						ROS_INFO_THROTTLE(4,"|UAV%d| :: formation time refreshed! ",global_id);
					}//ros::Time::now();
				else
					{	
						if(global_type_temp.type == 3)
						{ROS_INFO_THROTTLE(4,"|UAV%d| :: formation time timeout!  check!",global_id);
						}
						uav_code.header.stamp = ros::Time::now();
					}	
					     boost::mutex::scoped_lock type_lock1(type_mutex);
							     if(global_type_temp.type == 1)
							     {	
							     	boost::mutex::scoped_lock lock1(pos_mutex);
							     	ref_data[0]=xyz.pos[0 + 3*(global_id-1)];
								 	ref_data[1]=xyz.pos[2 + 3*(global_id-1)];
								 	ref_data[2]=-xyz.pos[1 + 3*(global_id-1)];
								 	tan_x = 2*(xyz.q[3 + 4*(global_id-1)]*xyz.q[1 + 4*(global_id-1)] - xyz.q[0 + 4*(global_id-1)]*xyz.q[2 + 4*(global_id-1)]);
								    tan_y = 1-2*(xyz.q[2 + 4*(global_id-1)]*xyz.q[2 + 4*(global_id-1)]+xyz.q[1 + 4*(global_id-1)]*xyz.q[1 + 4*(global_id-1)]);
							// 	 				    pos_status1.q1 = -xyz.q[0 + 4*(global_id-1)];
			    // pos_status1.q2 = xyz.q[2 + 4*(global_id-1)];
			    // pos_status1.q3 = xyz.q[1 + 4*(global_id-1)];
			    // pos_status1.q4 = xyz.q[3 + 4*(global_id-1)];
								 	ref_data[3]=-atan2(tan_x,tan_y);
								 	lock1.unlock();
							     }
							     type_lock1.unlock();
							boost::mutex::scoped_lock possp_lock(pos_sp_mutex);
	...
	while(ros::ok())
		{	
			...
			boost::mutex::scoped_lock pos_lock(pos_mutex);
				pos_status1.header.stamp = xyz.header.stamp;
			    ...
			    pos_status1.q3 = xyz.q[1 + 4*(global_id-1)];
			    pos_status1.q4 = xyz.q[3 + 4*(global_id-1)];
			    // x = 2*(pos_status1.q4*pos_status1.q3 + pos_status1.q1*pos_status1.q2);
			    // y = 1-2*(pos_status1.q2*pos_status1.q2+pos_status1.q3*pos_status1.q3);
			    // yawy = atan2(x,y);
			    // printf("%.2f\n",yawy);
